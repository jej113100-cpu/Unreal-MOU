// MOU 채팅 - (구) 전체 채팅 UI 위젯.
//
// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ ★★ 지금 이 위젯은 **어디에서도 자동으로 뜨지 않는다.** (2026-08-24, M9)  ║
// ╚══════════════════════════════════════════════════════════════════════════╝
//
// [무슨 일이 있었나]
//   로비의 소통 창구가 메신저(UMessengerWidgetBase, 1:1 DM)로 바뀌었다.
//   전체 채팅은 그 설계에 없으므로 ULoginWidgetBase 가 더 이상 띄우지 않는다
//   (bShowChatWidgetOnSuccess 기본값이 false 로 내려갔다).
//
// [그런데 왜 안 지웠나 — 이 폴더에 있는 이유]
//   여기 있는 것이 **인게임 죽은사람 채팅에 거의 그대로 필요하다**:
//     · 채팅 로그 스크롤 + 줄 추가 + 최대 줄 수 관리
//     · 입력창 + 엔터 전송 + 포커스 처리
//     · 토글 키 바인딩, PIE 다중 창 대응(TMap<World, Widget>)
//
//   죽은사람 채팅은 "누가 받는가" 만 다르고 화면은 이것과 같다. 지우고 나중에
//   다시 만드는 것보다 남겨두는 편이 싸다. 그래서 Chat/InGame/ 으로 옮겨
...
//   적혀 있다 — 요약하면 (1) 기록이 필요 없고 (2) 같은 방 사람끼리 하는 말이
//   서버를 두 번 건너갈 이유가 없고 (3) 리슨서버가 채팅 서버와 끊기면 게임 내
//   채팅이 죽는 새 실패 모드가 생긴다.
//
//   생사 판정은 UVoiceComponent::IsPlayerVoiceDead() 를 **재사용**한다.
